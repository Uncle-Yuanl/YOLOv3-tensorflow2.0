...
class Dataset():
        ...
        with tf.device('/cpu:0'):
            ...
            if self.batch_count < self.num_batchs:
                ...
                while num < self.batch_size:
                    ...
                    image, bboxes = self.parse_annotations(annotation)
                    label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = self.preprocess_true_bboxes(bboxes)

                    # 将每次处理一张图片的数据，放入batch数据中
    ...
    def preprocess_true_bboxes(self, bboxes):
        """
        处理一个annotation，即一张图片
        经过parse_annotation的bbox shape = (None, 5)
        ...
        for bbox in bboxes:
            ...
            bbox_xywh = np.concatenate(((bbox_coor[2:] - bbox_coor[:2]) / 2,
                                        bbox_coor[2:] - bbox_coor[:2]), axis=-1)

            # 一个gt_bbox放缩到三种尺度(feature map)上，分别标记，需要扩展维度
            ...
            for i in range(3):
                ...
                # 一个scale中三个anchor_box
                iou_mask = iou_scale > 0.3

                # 一个scale中的三个anchor_box，要有一个满足iou>0.3(框到了目标)，
                # 则对应scale(feature_map)对应位置上的标记为1
                if np.any(iou_mask):
            ...
            # 三种scale 9个anchor_bbox都没有框到目标，就选iou最大的
            if not exist_positive:
                best_anchor_ind = np.argmax(np.array(iou).reshape(-1), axis=-1)
                # 确定best_anchor_ind对应的scale
                best_scale = int(best_anchor_ind / self.anchor_per_scale)
                # 确定对应sacle中最好的anchor_box
